...
vector< myNode > getNeighbours(myNode &curNode, graph &g, vector< position > &goals) {
	...
	sort(neighbours.begin(), neighbours.end(),compareNode);
	return neighbours;
}

string BFS(graph &g, position &source, vector < position > &goals) {
	...
	while(!statesQueue.empty()) {
		...
		if(g[curNode.p.first][curNode.p.second] == 2) return curNode.ansStr;
...
int main() {
	...
	while(t--) {
		...
		cin >> x >> y;
		position source = {x,y};
		string solution = BFS(g, source, goals );
		cout << solution;
	}
}
